...
`APIConfigure` 프로토콜을 따르면 됩니다. `baseURL`은 자동으로 계산됩니다.

### API 정의

각 API 엔드포인트를 타입으로 정의합니다.
...
`APIDefinition` 프로토콜만 따르면 됩니다. 나머지는 기본 구현이 제공됩니다.
...
단 세 줄입니다. URLSession을 직접 쓰면 최소 20줄은 될 코드입니다.

---

## HTTP 메서드

모든 표준 HTTP 메서드를 지원합니다.
...
`NetworkError` 열거형으로 모든 네트워크 에러를 체계적으로 처리합니다.
...
`ExponentialBackoffRetryPolicy`는 지수 백오프를 자동으로 적용합니다.

- 1차 재시도: ~1초 대기
- 2차 재시도: ~2초 대기
- 3차 재시도: ~4초 대기

지터(jitter)를 추가해서 서버 부하를 분산시킵니다.
...
API 정의와 비즈니스 로직이 깔끔하게 분리됩니다.

---

## 다운로드 (InnoNetworkDownload)

대용량 파일 다운로드는 별도 모듈을 사용합니다.
...
백그라운드 다운로드와 재개 가능한 다운로드를 지원합니다.
