...
- 의존성(패키지/타겟) 변경 시 Resolve를 다시 진행해야 하는 경우가 있습니다.
...
- CocoaPods/Carthage 연동은 SPM 대비 제약이 있습니다. (사용이 완전히 불가능한 것은 아닙니다)
- (2024년 상반기 기준) 4 버전 전환 초기에는 잦은 수정과 일부 링킹 이슈를 겪을 수 있었습니다.
...
1. 위에서 말씀드린것과 같이, 당시 저희 프로젝트 구성에서는 파일/타겟 변경 후 Resolve를 자주 다시 수행해야 하는 문제가 지속적으로 발생하였습니다.
...
이부분은 `tuist edit` 을 통해 열린 xcodeproj 를 닫은 후, `tuist generate` 로 워크스페이스를 생성하고 Xcode 빌드(또는 `tuist build`)로 확인할 수 있습니다.
